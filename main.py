...
async def create_questions(question: QuestionBase, db: db_dependency):
    ...
    for choice in question.choices:
        db_choice = models.Choices(choices_text=choice.choices_text,
...
async def create_slider(slider: imagesliderBase, db: db_dependency):
    db_slider = models.Slider(image_name=slider.image_name,
                              image_url=slider.image_url, shop_url=slider.shop_url)
    db.add(db_slider)
    db.commit()
...
async def create_slider(slider: addsliderBase, db: db_dependency):
    db_slider = models.AddSlider(
        image_name=slider.image_name, image_url=slider.image_url, shop_url=slider.shop_url)
    db.add(db_slider)
    db.commit()
    db.refresh(db_slider)
...
async def product_(tags: str, db: db_dependency):
    queryData = db.execute(text(
        "SELECT * FROM Product as p where p.tags=:tags").bindparams(bindparam("tags", tags)))
...
async def create_footer(footer: footerBase, db: db_dependency):
    ...
    footer_category = db.execute(text(
        "SELECT * FROM FooterCategory as f where f.name=:request_name").bindparams(bindparam("request_name", request_name)))
    ...
    if footer_category is None:
        ...
        raise HTTPException(
            status_code=400, detail="Same Footer Name Already Exist")
...
async def footer_list(db: db_dependency):
    ...
    if not result:
        raise HTTPException(
            status_code=404, detail="Active Footer is not found")
...
async def discount_list(db: db_dependency):
    ...
    if not result:
        raise HTTPException(
            status_code=404, detail="Active Discount is not found")
...
async def category_list(db: db_dependency):
    ...
    if not result:
        raise HTTPException(
            status_code=404, detail="Active Category is not found")
...
async def brand_list(db: db_dependency):
    ...
    if not result:
        raise HTTPException(
            status_code=404, detail="Active Brand is not found")
...
async def model_list(db: db_dependency):
    ...
    if not result:
        raise HTTPException(
            status_code=404, detail="Active Model is not found")
...
async def model_find(brandId: int, db: db_dependency):
    request_id = brandId
    model_list = db.execute(text(
        "SELECT * FROM ProductModel as m where m.brand_id=:request_id").bindparams(bindparam("request_id", request_id)))
